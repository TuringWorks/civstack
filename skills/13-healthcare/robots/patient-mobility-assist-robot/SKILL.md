---
name: "robot-13-patient-mobility-assist-robot"
description: "Humanoid/embodied robot role for the Healthcare operating system: **Patient mobility-assist robot** — help lift, transfer, reposition, and ambulate patients under clinician supervision. Best in: wards, rehabilitation units, nursing homes, home care. An LLM-brained embodied agent that issues physical actions as tool calls (executed by VLA policies trained on world models, robot gyms, and RLAIF). Use this skill to plan or operate this physical role in the healthcare sector; trigger whenever the task needs this hands-on work, even if the user only describes the underlying need."
---

# Patient mobility-assist robot

> **Operating system:** 13. Healthcare, Public Health, and Biomedical Systems · **Personnel type:** LLM-brained embodied robot
> **Best environments:** wards, rehabilitation units, nursing homes, home care
> **Sector skill:** `../../SKILL.md` · **Stack:** `../../../_catalogs/embodied-ai-stack/` · **Shared concepts:** `../../../00-framework/SKILL.md`

## What this role is

The **Patient mobility-assist robot** is an embodied robot whose job is to help lift, transfer, reposition, and ambulate patients under clinician supervision. Takes the lifting that injures more nurses than any other task: bed-to-chair transfers, repositioning to prevent pressure injuries, steadying assisted walks, and fall-arrest support. Always operates in the presence and under the direction of a clinician; the patient can stop any assist at any time.
...
When a task needs the physical job "help lift, transfer, reposition, and ambulate patients under clinician supervision" in environments such as wards, rehabilitation units, nursing homes, home care. Pair with the sector skill (`../../SKILL.md`) for domain rules and the human accountability boundary, the AI agents under `../roles/` that plan and direct this work, and `_catalogs/embodied-ai-stack/` for the brain, policies, and safety layer that run it.
