---
name: capopt-model-distillation-compression-engineer
description: Capability/optimization role: **Model distillation & compression engineer** (Human engineering role (AI/robotics)) — distills large models into SLMs and tiny LMs and compresses them (quantization, pruning, sparsity) for on-device inference. Part of the layer that decides *how* robot and machine capabilities are built — across model tiers (LLM, SLM, tiny LM, deterministic) and many training methods (imitation, model-based/offline RL, RLHF/RLAIF, sim-to-real, distillation, classical control, formal methods). Use this skill when choosing or building how a capability is trained, optimized, or run on-device, even if the user only describes the underlying need. Works under a edge-AI lead.
---

# Capability & Optimization — Model distillation & compression engineer

> **Layer:** Capability / optimization spectrum (how robots and machines are made capable) · **Type:** Human engineering role (AI/robotics)
> **Human supervisor:** edge-AI lead · **Used by:** `../embodied-ai-stack/`, `../autonomous-fleet-ops/`, robot & machine skills · **Shared concepts:** `../../00-framework/SKILL.md`

## What this role is

The **Model distillation & compression engineer** distills large models into SLMs and tiny LMs and compresses them (quantization, pruning, sparsity) for on-device inference. Turns an exhaustive but expensive LLM brain into efficient on-device models — the key to running capability within power and latency budgets.
...
Use it when a task calls for this work: distills large models into SLMs and tiny LMs and compresses them (quantization, pruning, sparsity) for on-device inference. Pair with the robot skills (`_catalogs/humanoid-robots/`, `<sector>/robots/`) and machine skills (`_catalogs/autonomous-machines/`, `<sector>/autonomous/`) whose capabilities are being trained, optimized, or deployed.
