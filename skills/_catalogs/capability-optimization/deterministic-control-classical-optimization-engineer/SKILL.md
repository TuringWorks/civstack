---
name: capopt-deterministic-control-classical-optimization-engineer
description: Capability/optimization role: **Deterministic control & classical-optimization engineer** (Human engineering role (AI/robotics)) — implements the non-learned controllers and optimizers — PID, MPC, state machines, planners, convex/MILP — for hard-real-time and safety-critical loops. Part of the layer that decides *how* robot and machine capabilities are built — across model tiers (LLM, SLM, tiny LM, deterministic) and many training methods (imitation, model-based/offline RL, RLHF/RLAIF, sim-to-real, distillation, classical control, formal methods). Use this skill when choosing or building how a capability is trained, optimized, or run on-device, even if the user only describes the underlying need. Works under a controls lead.
---

# Capability & Optimization — Deterministic control & classical-optimization engineer

> **Layer:** Capability / optimization spectrum (how robots and machines are made capable) · **Type:** Human engineering role (AI/robotics)
> **Human supervisor:** controls lead · **Used by:** `../embodied-ai-stack/`, `../autonomous-fleet-ops/`, robot & machine skills · **Shared concepts:** `../../00-framework/SKILL.md`

## What this role is

The **Deterministic control & classical-optimization engineer** implements the non-learned controllers and optimizers — PID, MPC, state machines, planners, convex/MILP — for hard-real-time and safety-critical loops. Not everything should be learned: deterministic controllers are verifiable, cheap, and reliable, and form the safety backbone beneath the learned brain.
...
Use it when a task calls for this work: implements the non-learned controllers and optimizers — PID, MPC, state machines, planners, convex/MILP — for hard-real-time and safety-critical loops. Pair with the robot skills (`_catalogs/humanoid-robots/`, `<sector>/robots/`) and machine skills (`_catalogs/autonomous-machines/`, `<sector>/autonomous/`) whose capabilities are being trained, optimized, or deployed.
