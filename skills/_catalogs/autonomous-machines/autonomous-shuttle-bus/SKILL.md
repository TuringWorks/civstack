...
When a task needs the physical job "move passengers on fixed or flexible low-speed routes" in environments such as campuses, downtowns, transit feeders. Pair with the relevant operating-system skill (01–23) for domain rules and the human accountability boundary, and with `_catalogs/embodied-ai-stack/` for the roles that build, operate, and keep it safe.
