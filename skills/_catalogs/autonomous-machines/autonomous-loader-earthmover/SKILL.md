...
When a task needs the physical job "load, dig, grade, and move material on sites" in environments such as construction sites, quarries, ports, yards. Pair with the relevant operating-system skill (01–23) for domain rules and the human accountability boundary, and with `_catalogs/embodied-ai-stack/` for the roles that build, operate, and keep it safe.
