...
When a task needs the physical job "harvest, sort, pack, inspect" in environments such as greenhouses, controlled farms. Pair with the relevant operating-system skill (01–23) for domain safety rules and the human accountability boundary, and with `_catalogs/embodied-ai-stack/` for the roles that build and operate the brain, policies, and safety layer.
